...
import CssExtractor, { DECLARATION_REGEXP, REGEXP } from '../../src/lib/variables/strategies/css-strategy'
...
import { generateDecorationType } from '../../src/lib/styled-colors-config'
...
describe('Test variables declaration Regex', () => {
   ...
   it('Should not match (css variables)', () => {
...
describe('Test variables use regexp', () => {
   // css variables (works for postcss too)
   it('Should match (css variables)', () => {
...
describe('Test decoration generation', () => {
   it('The generated decoration should contain (var)', async () => {
   ...
   it('The generated decoration should surround the variable use', async () => {
      // const extractor = new CssExtractor();
      await CssExtractor.extractDeclarations('fileName', [{ line: 0, text: '--darken: blue' }])
