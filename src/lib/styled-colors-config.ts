import type { Extension, TextEditorDecorationType, WorkspaceConfiguration } from 'vscode'
import { DecorationRangeBehavior, extensions, window, workspace } from 'vscode'
   ...
   const filesExtensions = extensions.all.reduce((acc, extension: Extension<unknown>) => {
      if (extension.packageJSON?.contributes?.languages) {
         extension.packageJSON.contributes.languages.forEach((language) => {
            if (languagesConfig.includes(language.id) && language.extensions) {
               acc = [
                  ...acc,
                  ...language.extensions,
               ]
...
export { StyledColorsConfig, getStyledColorsConfig, generateDecorationType }
